...
from utils.utils_checkpoint import *
...
def window_preds_to_frame_labels(preds, T_total, *, window_T, stride, weight='flat'):
    """
    preds: [N_win] 윈도우 예측(정수 클래스 ID)
    T_total: 프레임 수(len(df_in)) - 윈도우 만들 때 사용한 '그 df'의 길이와 같아야 함
    window_T, stride: 윈도우 생성 값과 동일
    weight: 'flat' 또는 'tri'(가중치 삼각형) – 겹칠 때 가운데에 가중 더 주고 싶으면 'tri'
    ...
    assert len(starts) == len(preds), f"윈도우 개수 불일치: {len(starts)} vs {len(preds)}"
    for s, cls in zip(starts, preds):
        e = s + window_T
        votes[s:e, int(cls)] += w
...
def window_preds_to_frame_labels(preds, T_total, *, window_T, stride,
    ...
    # 길이 불일치 시 안전하게 맞추기(자세한 이유는 아래 참고)
...
def attach_pred_to_df(df_in: pd.DataFrame, preds, *, window_T, stride, starts=None) -> pd.DataFrame:
    """
    df_in: 테스트 윈도우를 만들 때 사용한 '그' 프레임 테이블 (보통 out_of_play 제외 + index reset 된 것)
    preds: 테스트 윈도우 예측(정수 ID) – make_windows 순서와 동일
...
def save_inference_bundle(save_dir, tag, state_dict, scaler, meta: dict):
    """
    추론용 번들 저장: 가중치(.pt) + 스케일러(.pkl) + 메타(.json)
    - state_dict: model.state_dict() or 그 deepcopy
    - scaler: StandardScaler (per-fold로 fit된 것), 없으면 None
    - meta: id2label, feature_cols, window_T, stride, 등
    """
    p = Path(save_dir); p.mkdir(parents=True, exist_ok=True)
    torch.save(state_dict, p / f"{tag}.weights.pt")
    if scaler is not None:
        joblib.dump(scaler, p / f"{tag}.scaler.pkl")
    with open(p / f"{tag}.meta.json", "w", encoding="utf-8") as f:
...
if __name__ == '__main__':
    ...
    # df_test, ev_all_test = build_frame_df(xy_objects, events, possession, ballstatus,teamsheets=teamsheets, pitch=pitch, FR=FR, params=params)

    keep_states = ('passing','dribbling','shooting','idle')
    df_all, ev_tables = build_multi_match_dataset(path, FR=25.0, params=params, keep_only_states=keep_states)
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(X_cv, y_cv, groups_cv)):
        ...
        #model = GRUBaseline(in_feat=in_feat, n_classes=n_classes).to(device)
        ...
        criterion = nn.CrossEntropyLoss(weight=class_weights.to(device))  # 필요시 FocalLoss로 교체
        ...
        best_f1 = train_with_progress(
            ...
            use_tqdm_train=True,   # ← 학습에만 tqdm 켬
            use_tqdm_val=False     # ← 검증은 깔끔하게 로그만
        )
    ...
    df_te_pred[(df_te_pred['frame']<T1) & (df_te_pred['time_s']>=60) & (df_te_pred['time_s']<=90)],
    pitch=pitch, key='pred_state',
    title='1st half 60–90s (Pred, segmented)',
    save_path=f"plots/{TEST_ID}_1st_60-90s_pred_GRU.png",  # 원하는 경로/이름
    dpi=200, show=False
    )
